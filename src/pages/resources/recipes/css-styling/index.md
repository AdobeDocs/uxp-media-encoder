...
Media Encoder supports theme awareness, which means that your plugin's UI can adapt to the user's chosen application theme—either, **Light**, **Dark**, or **Darkest**. In Media Encoder UXP plugins, the `document` object implements a `theme` property with a `getCurrent()` method that returns the current theme as a string.
...
Spectrum CSS variables that are [available in other Creative Cloud applications](https://developer.adobe.com/photoshop/uxp/2022/guides/theme-awareness/#built-in-theme-colors) (for example, `--uxp-host-text-color`) are **not yet supported** in Media Encoder. They will be added in a future release.
...
UXP does not support all CSS properties. For example, CSS Grid Layout is not available. Check the [CSS reference](../../../uxp-api/index.md) for a complete list of supported properties.

### CSS Preprocessors

UXP only understands standard CSS. If you want to use preprocessors like **Sass** or **SCSS**, you must transpile them to CSS before bundling your plugin. This requires build tools like Webpack and a slightly different [debugging workflow](../../../plugins/index.md).
...
- [CSS Reference](../../../uxp-api/index.md): complete list of supported CSS properties.
- [Spectrum Web Components](../../../uxp-api/index.md) Reference.
