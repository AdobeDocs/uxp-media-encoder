...
By default, UXP plugins can't connect to the internet; this helps keep users safe by preventing unwanted or hidden network activity. If your plugin needs to fetch data, load images, or use WebSockets, **you must declare** the right [network permissions](../../../plugins/index.md).

In your plugin's `manifest.json`, use the [`requiredPermissions.network`](../../../plugins/index.md) object to lists the domains your plugin can access.
...
The [`fetch()`](../../../uxp-api/index.md) API is the simplest and most flexible way to make network requests. It's modern, asynchronous, and built into the UXP global scope—no `require()` needed.
...
We have [mentioned this before](../../../get-started/index.md#uxp-is-not-a-browser); some features you're familiar with are not available. For example, you can't use the `TextDecoder` class to read a data stream.
...
async function connectToServer() {
  try {
    ...
    socket.onopen = () => {
      ...
      socket.send("Hello from Media Encoder plugin!");
    };
...
| Connection fails only on macOS           | HTTPS required                     | Media Encoder disallows http:// URLs on macOS                                   |
...
- [`fetch`](../../../uxp-api/index.md) API Reference.
- [`XMLHttpRequest`](../../../uxp-api/index.md) Reference.
- [`WebSocket`](../../../uxp-api/index.md) Reference.
- [Manifest Permissions](../../../plugins/index.md).
- [Network Permission Details](../../../plugins/index.md).
