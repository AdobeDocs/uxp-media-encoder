...
This section contains frequently asked questions about UXP and Media Encoder. Hybrid Plugins specific questions are [found here](../../plugins/index.md).
...
Ensure that Media Encoder is running. If it is, check that the host application's Developer Mode is enabled; follow the instructions in the [this guide](../../plugins/index.md#prerequisites).

\<!-- 🎨 User Interfaces --\>

#### How can I use Spectrum Web Components in my plugin?

You can use Spectrum Web Components in your plugin by following the instructions in the [Spectrum Web Components](../../uxp-api/index.md) guide.
...
Please refer to [these troubleshooting steps](../../plugins/index.md) for more details.

#### My `.ccx` package is being rejected when submitted to the Creative Cloud Marketplace. What could be wrong?

Check if the [`host`](../../plugins/index.md) property in your `manifest.json` is an object, not an array. If it's an array, the validation will fail. See [Host Applications](../../plugins/index.md) for details.

#### Should I use an Array or an Object for the `host` property in the `manifest.json`?

The `host` property must be a single [`HostDefinition`](../../plugins/index.md) Object for production. Arrays are only allowed during development for convenience. See the [Package a UXP plugin](../../plugins/index.md) section for details.
